            ...
            events.forEach((event) => {
                ...
                eventDiv.innerHTML = `
                    <div class="rounded-lg h-64 overflow-hidden">
                        <img src="${
                            sportImages[event.sport.toLowerCase()]
                        }"" alt="content" class="object-contain object-center h-full w-full">
                    </div>
                    <h2 class="text-xl font-medium title-font text-gray-900 mt-5">${sanitizeInput(event.title)}</h2>
                    <h4 class="text-md font-medium title-font text-gray-600 mt-1">${sanitizeInput(event.sport)}</h4>
                    <h4 class="text-md font-medium title-font text-gray-600 mt-1">${sanitizeInput(event.date)}</h4>
                    <p class="text-base leading-relaxed mt-2">${sanitizeInput(event.description)}</p>
                    <a href="/eventDetail.html?id=${sanitizeInput(
                        event.id
                    )}" class="text-indigo-500 inline-flex text-blue-600 items-center mt-3">Détails
